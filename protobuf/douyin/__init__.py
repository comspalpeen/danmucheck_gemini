...
class ChatMessage(betterproto.Message):
    ...
    chat_tags_list: List[str] = betterproto.string_field(19)
...
class GiftTrayInfo(betterproto.Message):
    ...
    tray_type: int = betterproto.uint64_field(5)
    """[修复] 补充 Field 5"""

    tray_right_img: "Image" = betterproto.message_field(6)
    """[修复] 补充 Field 6"""

    tray_image_base: "Image" = betterproto.message_field(9)
    duration: int = betterproto.uint64_field(12)
    tray_effect: "GiftTrayInfoTrayEffect" = betterproto.message_field(15)


@dataclass(eq=False, repr=False)
class GiftTrayInfoTrayEffect(betterproto.Message):
    image: "Image" = betterproto.message_field(1)
    duration: int = betterproto.uint64_field(2)
    time: int = betterproto.uint64_field(5)
...
@dataclass(eq=False, repr=False)
class LinkMicBattleFinishMethod(betterproto.Message):
    """
    ...
    common: "Common" = betterproto.message_field(1)
    info: "BattleFinishInfo" = betterproto.message_field(2)
    contributors: List["BattleContributors"] = betterproto.message_field(3)
    """
    Field 3: [修正] 贡献榜 (Contributors)
     结构: anchor_id -> list of items (扁平结构)
    """

    scores: List["BattleScore"] = betterproto.message_field(4)
    """Field 4: 分数结算"""

    anchors: List["BattleArmy"] = betterproto.message_field(12)
    """
    Field 12: [修正] 主播信息 (Anchors)
     结构: anchor_id -> list of items (嵌套结构: item -> user)
    """

    skin_config: "BattleSkinConfig" = betterproto.message_field(15)
    event_tracking_info: str = betterproto.string_field(18)


@dataclass(eq=False, repr=False)
class BattleFinishInfo(betterproto.Message):
    battle_id: int = betterproto.uint64_field(2)
    start_time_ms: int = betterproto.uint64_field(3)
    duration: int = betterproto.uint64_field(4)
    title_config: "BattleTitleConfig" = betterproto.message_field(17)
    punish_config: "BattlePunishConfig" = betterproto.message_field(18)
    battle_config_json: str = betterproto.string_field(39)
    status: int = betterproto.uint64_field(40)
...
class BattleScore(betterproto.Message):
    ...
    score: int = betterproto.uint64_field(1)
    user_id: int = betterproto.uint64_field(2)
    rank: int = betterproto.uint64_field(16)
    description: str = betterproto.string_field(18)
    team_total_score: int = betterproto.uint64_field(19)
    win_status: int = betterproto.uint32_field(20)
    status_21: int = betterproto.uint32_field(21)
    part_of_team_id: int = betterproto.uint64_field(24)
...
class BattleArmy(betterproto.Message):
    ...
    anchor_id: int = betterproto.uint64_field(1)
    list: List["BattleAnchorItem"] = betterproto.message_field(2)
    """Field 2: 主播列表"""
...
@dataclass(eq=False, repr=False)
class BattleAnchorItem(betterproto.Message):
    user: "BattleUserInfo" = betterproto.message_field(1)
    """这是一个嵌套对象"""
...
@dataclass(eq=False, repr=False)
class BattleContributors(betterproto.Message):
    """对应 Field 3: 贡献榜列表 (按 Team/Anchor ID 分组)"""

    anchor_id: int = betterproto.uint64_field(1)
    list: List["BattleContributorItem"] = betterproto.message_field(2)
    """Field 2: 贡献者列表 (大哥)"""
...
@dataclass(eq=False, repr=False)
class BattleContributorItem(betterproto.Message):
    """贡献榜单项 (大哥)"""

    id: int = betterproto.uint64_field(1)
    nickname: str = betterproto.string_field(2)
    avatar: "Image" = betterproto.message_field(3)
    score: int = betterproto.uint64_field(4)
    id_str: str = betterproto.string_field(5)
    rank: int = betterproto.uint64_field(6)
    """可能存在的其他字段"""

    delta: int = betterproto.uint64_field(10)
...
@dataclass(eq=False, repr=False)
class BattleResultContributor(betterproto.Message):
    """详细贡献者列表 (对应 Field 12)"""

    anchor_id: int = betterproto.uint64_field(1)
    items: List["BattleResultContributorItem"] = betterproto.message_field(2)


@dataclass(eq=False, repr=False)
class BattleResultContributorItem(betterproto.Message):
    user: "User" = betterproto.message_field(1)
    tag_map: "BattleContributorTag" = betterproto.message_field(3)
    """
    Field 3 是复杂结构，有时是 map，有时包含 Tag
     在 1vs7.txt 中是 {1:1, 2:3}
    """

    rank_index: int = betterproto.uint64_field(8)
    delta: int = betterproto.uint64_field(10)
    season_badge: "BattleRankSeasonBadge" = betterproto.message_field(12)


@dataclass(eq=False, repr=False)
class BattleContributorTag(betterproto.Message):
    key: int = betterproto.uint64_field(1)
    value: int = betterproto.uint64_field(2)


@dataclass(eq=False, repr=False)
class BattleRankSeasonBadge(betterproto.Message):
    icon_small: "Image" = betterproto.message_field(1)
    icon_big: "Image" = betterproto.message_field(2)
    title_str: str = betterproto.string_field(3)
    url_str: str = betterproto.string_field(4)
...
@dataclass(eq=False, repr=False)
class BattlePunishConfig(betterproto.Message):
    """惩罚配置 (对应 Field 18)"""
...
class BattleSetting(betterproto.Message):
    ...
    battle_id: int = betterproto.uint64_field(1)
    title: str = betterproto.string_field(2)
    scheme_url: str = betterproto.string_field(3)
    scheme_url_extra: str = betterproto.string_field(4)
    duration: int = betterproto.uint64_field(5)
...
@dataclass(eq=False, repr=False)
class WebcastLotteryEventNewMessage(betterproto.Message):
    common: "Common" = betterproto.message_field(1)
    lottery_id: int = betterproto.uint64_field(2)
    status: int = betterproto.uint64_field(3)
    start_time: int = betterproto.uint64_field(4)
    end_time: int = betterproto.uint64_field(5)
    calculation_time: int = betterproto.uint64_field(6)
    rule_page_url: str = betterproto.string_field(7)
    event_type: int = betterproto.uint64_field(8)
    extra_bytes: bytes = betterproto.bytes_field(10)
    """Field 9 skipped"""

    rule_info: List["LotteryRuleInfo"] = betterproto.message_field(11)
    lottery_type: int = betterproto.uint64_field(12)
    winner_count: int = betterproto.uint64_field(13)
    current_stock: int = betterproto.uint64_field(14)
    prize_name: str = betterproto.string_field(18)
    """15-17 skipped"""

    prize_description: str = betterproto.string_field(19)
    countdown: int = betterproto.uint64_field(20)
    content_list: List["LotteryContent"] = betterproto.message_field(21)


@dataclass(eq=False, repr=False)
class LotteryRuleInfo(betterproto.Message):
    """对应 Field 11: 规则键值对"""
...
@dataclass(eq=False, repr=False)
class LotteryContent(betterproto.Message):
    """对应 Field 21: 福袋内容/标签"""
